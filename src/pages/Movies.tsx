...
const Movies: React.FC = () => {
  ...
  const [searchLoading, setSearchLoading] = useState(false); // Nouveau état pour le loader de recherche
  ...
  // Nouvel useEffect pour la recherche automatique
  useEffect(() => {
    if (searchQuery.trim()) {
      setSearchLoading(true); // Activer le loader
    }
    ...
    const timeoutId = setTimeout(() => {
      ...
      setSearchLoading(false); // Désactiver le loader après le délai
    }, 500);

    return () => {
      clearTimeout(timeoutId);
      if (!searchQuery.trim()) {
        setSearchLoading(false); // S'assurer que le loader est désactivé si on efface le texte
      }
    };
  }, [searchQuery]);
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="text-center mb-8 sm:mb-12">
          <h1 className="text-3xl sm:text-4xl lg:text-6xl font-bold text-white mb-6 sm:mb-8">
            ...
            <form onSubmit={(e) => e.preventDefault()} className="max-w-md mx-auto mb-6">
